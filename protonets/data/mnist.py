...
# MNIST_DATA_DIR  = os.path.join(os.path.dirname(__file__), '../../data/mnist/data/MNIST_6rot.pkl.gz')
MNIST_DATA_DIR  = os.path.join(os.path.dirname(__file__), '../../data/mnist')
MNIST_CACHE = { }

def load_mnist_from_file(path=MNIST_DATA_DIR+"/data/MNIST_6rot.pkl.gz"):
    file = gzip.open(path, 'rb')
    domains = pickle.load(file, encoding='latin1') # Python3 must use `encoding=latin1`
    data = np.array([each[0] for each in domains]) # TODO reshape TODO check the label is right
    data = np.reshape(data, newshape=(6, 10, 100, 1, 16, 16))
    return  data # (6, 10, 100, 16, 16) # (rot, class, num, w, h) # TODO move to right place

MNIST_DATA = load_mnist_from_file()
...
def load_class_images(d):
    ...
    '''
    if d['class'] not in MNIST_CACHE:
        alphabet, character, rot = d['class'].split('/')
        image_dir = os.path.join(MNIST_DATA_DIR, 'data', alphabet, character)
        ...
        for sample in loader:
            MNIST_CACHE[d['class']] = sample['data']
            break # only need one sample because batch size equal to dataset length

    return { 'class': d['class'], 'data': MNIST_CACHE[d['class']] } # {'class': 'Aurek-Besh/character21/rot180', 'data': [20, 1, 28, 28]}

def load_class_images_odg(d):
    '''
    input: class name. eg. "num_1/rot060"
    return: eg. {'class': 'Aurek-Besh/character21/rot180', 'data': [20, 1, 28, 28], 'data_next': [20, 1, 28, 28]}
    '''
    #  input data = 

    def next_rot(c): # "num_1/rot060"
        y_class, rot = c.split('/')
        rot = rot[:3]+ "0" + str(int(rot[3:]) + 15)
        c = '/'.join([y_class, rot]) # TODO check the +, it seems there is error
        return c
    
    def turn_c_to_rot_class(c): # "num_1/rot060"
        y_class, rot = c.split('/')
        _, y_class_idx = y_class.split("_")
        rot = int(int(rot[-3:]) / 15)
        y_class = int(y_class[-1])
        return rot, y_class
    ...
    for c in [origin_class, next_class]:
        if c not in MNIST_CACHE:
            rot, y_class_idx = turn_c_to_rot_class(c)
            MNIST_CACHE[c] = torch.from_numpy(np.array(MNIST_DATA[rot][y_class_idx], np.float32, copy=False))
            # convert to tensor
    return {'class': origin_class, 'data': MNIST_CACHE[origin_class], 'data_next': MNIST_CACHE[next_class]} # {'class': 'Aurek-Besh/character21/rot180', 'data': [20, 1, 28, 28], 'data_next': [20, 1, 28, 28]}
...
def extract_episode_odg(n_support, n_query, d):
    ...
    example_inds = torch.randperm(n_examples)[:(n_support+n_query)] # make index and split
    support_inds = example_inds[:n_support]
    query_inds = example_inds[n_support:]

    xs = d['data'][support_inds]
    xq = d['data_next'][query_inds] # note here # TODO 1. Random Case, 2. Exclusive Case 3. Pair Case
...
def load(opt, splits):
    split_dir = os.path.join(MNIST_DATA_DIR, 'splits', opt['data.split'])
    ...
    for split in splits:
        ...
        if 'odg' in split:
            sampler = ODGSampler(len(ds), n_way, n_episodes, split) # split can be 'odg_train' 'odg_test' # TODO add domain_num

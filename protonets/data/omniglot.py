...
def load(opt, splits):
    ...
    ret = { }
    for split in splits:
        if split in ['val', 'test'] and opt['data.test_way'] != 0: # get way value
            n_way = opt['data.test_way']
        else:
            n_way = opt['data.way']

        if split in ['val', 'test'] and opt['data.test_shot'] != 0: # get shot value
            n_support = opt['data.test_shot']
        else:
            n_support = opt['data.shot']

        if split in ['val', 'test'] and opt['data.test_query'] != 0: # get query value
        ...
        transforms = [partial(convert_dict, 'class'), # convert to {class: "Angelic/character01/rot000"}
                      ...
                      # TODO find the range of rot
                      # TODO return {'class': 'Aurek-Besh/character21/rot180', 'data': [20, 1, 28, 28], 'data_next': [20, 1, 28, 28]}
                      # TODO return {'class': , 'xs': , 'xq': }
